...
t1 = T(2).^(-100:1//10:-1)
t2 = (-5000:4999) / T(5000)
validation_points = -cispi.(union(collect(t1), collect(-t1), collect(t2)));
t1 = T(2).^(-52:1//10:-1)
t2 = (-5000:4999) / T(5000)
...
test_cases = [
    ...
    (desc=L"|z + 1|", fun=z->abs(z + 1 )),
    (desc=L"|z + 1 + 10^{-6}|", fun=z->abs(z + 1 + 1e-6)),
    (desc=L"\log(z + 1 + 10^{-6})", fun=z->log(complex(z + 1 + 1e-6))),
